...
TukeyHSD(fit1, which="as.factor(Elevation)")
#diff          lwr       upr     p adj
...
compare_means(DustComplexity ~ Elevation2, data=pw_comp, method="anova") # won't take as.factor(Elevation) as input
...
coef(summary(its1.fit1))[8] # pvalue 
...
coef(summary(its1.sr.fit1))[8] # pvalue 
...
fig.its1.sr.fit1<-ggplot(pw_comp, aes(x = ITS1_Species_Richness, y = DustComplexity)) + 
  geom_point(aes(color=Elevation), size=3) + theme_classic() + saturation(scale_colour_gradientn(colours=fair_cols,limits=c(400,2700),breaks = c(500,1250,2000,2600),labels=c("400","1100","2000","2700")), 0.9) +
  ...
  stat_regline_equation(label.y = 1.05,label.x=700)
...
fig.Bac.fit1<-ggplot(pw_comp, aes(x = Bac_Shannon_Diversity, y = DustComplexity)) + 
  ...
  stat_regline_equation(label.y = 1.05,label.x=280)
...
fig.Bac.fit1a<-ggplot(pw_comp.bacclean, aes(x = Bac_Shannon_Diversity, y = DustComplexity)) + 
  ...
  stat_regline_equation(label.y = 1.05,label.x=200)
...
fig.Bac.sr.fit1<-ggplot(pw_comp, aes(x = Bac_Species_Richness, y = DustComplexity)) + 
  ...
  stat_regline_equation(label.y = 1.05,label.x=6000)
