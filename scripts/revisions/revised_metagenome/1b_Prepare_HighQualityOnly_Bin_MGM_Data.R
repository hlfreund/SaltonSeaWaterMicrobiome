...
# Reformat the names of bins and samples
...
# Drop bins below 90% completeness
gbins.qual<-as.data.frame(read_xlsx("data/Metagenomes/Analysis/SSW_Samples_GoodBins.xlsx", sheet="SSW_Samples_GoodBins"))
gbins.qual$Bin_ID <- paste(gbins.qual$SampleID, gbins.qual$Bin_Num, sep = ".")
gbins.qual$Bin_ID %in% bin_fxns.cov$Bin_ID # sanity check that good bins Bin_ID and the bin cove data matches!

# subset out only high quality bins based on bin completeness (already have contamination < 5%)
hqaulbins<-gbins.qual[gbins.qual$Completeness>=90,] # subset out only high quality bins per Bower et al 2017

# drop any bins that do not have >= 90% completeness
dim(bin_fxns.cov)
bin_fxns.cov<-bin_fxns.cov[bin_fxns.cov$Bin_ID %in% hqaulbins$Bin_ID,]
dim(bin_fxns.cov) # sanity check that we did drop bins
...
# drop any bins that do not have >= 90% completeness
dim(mag_tax)
mag_tax<-mag_tax[mag_tax$Bin_ID %in% hqaulbins$Bin_ID,]
dim(mag_tax) # sanity check that we did drop bins
...
colorset1 = reshape2::melt(c(August.2021="#ef781c",December.2021="#03045e",April.2022="#059c3f"))
...
# write new taxa table for manuscript
mag_tax.df<-as.data.frame(mag_tax)
write.table(mag_tax.df, file="data/Metagenomes/Analysis/HighQuality_MAGs_Taxonomy_Only.txt", sep="\t", quote=F, col.names=T)
