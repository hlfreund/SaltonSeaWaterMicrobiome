...
#### ITS1 - Alpha Diversity + Species Richness (raw data) ####
# ## H <- diversity() -- Shannon entropy function
## exp(H) -- Shannon diversity (exp of entropy)

Shan_ent.its1<-vegan::diversity(its1_otu_table, index="shannon") # Shannon entropy
...
#### 16S - Alpha Diversity + Species Richness (raw data) ####
# ## H <- diversity() -- Shannon entropy function
## exp(H) -- Shannon diversity (exp of entropy)

Shan_ent.16s<-vegan::diversity(bac_otu_table, index="shannon") # Shannon entropy
...
# create elevation factor variable because some commands refuse to take as.factor(Elevation) as input
...
hist(pw_comp$DustComplexity[pw_comp$Year==2014])
hist(pw_comp$DustComplexity[pw_comp$Year==2015])

hist(pw_comp$ITS1_Shannon_Diversity, freq=FALSE)
hist(pw_comp$ITS1_Shannon_Diversity[pw_comp$Year==2014])
hist(pw_comp$ITS1_Shannon_Diversity[pw_comp$Year==2015])

hist(pw_comp$ITS1_Species_Richness, freq=FALSE)
hist(pw_comp$ITS1_Species_Richness[pw_comp$Year==2014])
hist(pw_comp$ITS1_Species_Richness[pw_comp$Year==2015])

hist(pw_comp$Bac_Shannon_Diversity, freq=FALSE)
hist(pw_comp$Bac_Shannon_Diversity[pw_comp$Year==2014])
hist(pw_comp$Bac_Shannon_Diversity[pw_comp$Year==2015])

hist(pw_comp$Bac_Species_Richness, freq=FALSE)
hist(pw_comp$Bac_Species_Richness[pw_comp$Year==2014])
hist(pw_comp$Bac_Species_Richness[pw_comp$Year==2015])
...
# shapiro.test(pw_comp$DustComplexity[pw_comp$Year==2014])
# shapiro.test(pw_comp$DustComplexity[pw_comp$Year==2015])
...
# shapiro.test(pw_comp$ITS1_Shannon_Diversity[pw_comp$Year==2014])
# shapiro.test(pw_comp$ITS1_Shannon_Diversity[pw_comp$Year==2015])
...
# shapiro.test(pw_comp$ITS1_Species_Richness[pw_comp$Year==2014])
# shapiro.test(pw_comp$ITS1_Species_Richness[pw_comp$Year==2015])
...
# shapiro.test(pw_comp$Bac_Shannon_Diversity[pw_comp$Year==2014])
# shapiro.test(pw_comp$Bac_Shannon_Diversity[pw_comp$Year==2015])
...
# shapiro.test(pw_comp$Bac_Species_Richness[pw_comp$Year==2014])
# shapiro.test(pw_comp$Bac_Species_Richness[pw_comp$Year==2015])
...
fit1<-aov(DustComplexity ~ as.factor(Elevation), data=pw_comp)
pairwise.t.test(pw_comp$DustComplexity, as.factor(pw_comp$Elevation), p.adjust.method='bonferroni')
#pairwise.adonis(pw_comp$DustComplexity, pw_comp$Elevation, p.adjust.m='bonferroni') # shows us variation for each sample to see which ones are different

summary(fit1)
#Df           Sum Sq Mean Sq    F value   Pr(>F)    
#Elevation2   3 0.7277 0.24258   14.74 7.06e-06 ***
#Residuals   27 0.4444 0.01646
Tuk1<-TukeyHSD(fit1, which="as.factor(Elevation)")
Tuk1$`as.factor(Elevation)`
write.csv(Tuk1$`as.factor(Elevation)`,"data/Sierra_DustComplexity_Elevation_TukeyHSD_ANOVA_3.1.22.csv",row.names=TRUE)

#diff          lwr       upr     p adj
#1100-400  0.17541640 -0.000132067 0.3509649 0.0502253
#2000-400  0.20607259  0.030524123 0.3816211 0.0168117
#2700-400  0.43987089  0.258160964 0.6215808 0.0000024
#2000-1100 0.03065619 -0.144892281 0.2062047 0.9633302
#2700-1100 0.26445449  0.082744561 0.4461644 0.0024645
#2700-2000 0.23379830  0.052088371 0.4155082 0.0079316

#plot(DustComplexity ~ Elevation, data=pw_comp)
#abline(aov(DustComplexity ~ Elevation, data=pw_comp))

# fit.0<-aov(DustComplexity ~ as.factor(Elevation), data=pw_comp)
# summary(fit.0)
# TukeyHSD(fit.0)
# Levene's test with one independent variable
## Levene's tests wehther variances of 2 samples are equal
...
fligner.test(DustComplexity ~ as.factor(Elevation), data = pw_comp)
...
compare_means(DustComplexity ~ Elevation2, data=pw_comp, method="anova",p.adjust.method = "bonferroni") # won't take as.factor(Elevation) as input

p.adj.dc.elev<-compare_means(DustComplexity ~ Elevation2, data=pw_comp, method="t.test",p.adjust.method = "bonferroni") # won't take as.factor(Elevation) as input# Note https://github.com/kassambara/ggpubr/issues/65

fit.test<-ggplot(pw_comp, aes(x = as.factor(Elevation), y = DustComplexity, fill=as.factor(Elevation))) + 
...
fit.test0<-ggplot(pw_comp, aes(x = as.factor(Elevation), y = DustComplexity, fill=as.factor(Elevation))) + 
  geom_boxplot() + theme_classic() + guides(fill = guide_legend(reverse=TRUE)) + 
  theme(axis.title.x = element_text(size=13),axis.title.y = element_text(size=13),legend.title.align=0.5, legend.title = element_text(size=13),axis.text = element_text(size=11),axis.text.x = element_text(vjust=1),legend.text = element_text(size=11)) +
  labs(title="Dust Complexity x Elevation",fill="Elevation (ft)")+ylab("Dust Complexity")+xlab("Elevation (ft)")+scale_fill_manual(values=saturation(fair_cols, 0.9))+stat_compare_means(method = "anova",label.y=1.5) +stat_compare_means(comparisons = list(c(1,2), c(2,3),  c(1,3),  c(3,4),  c(2,4),  c(1,4)), method="t.test", hide.ns = TRUE,label = "p.signif")
...
fit.testb.0<-ggplot(pw_comp, aes(x = as.factor(Elevation), y = DustComplexity, fill=as.factor(Elevation))) + 
  geom_boxplot() + theme_classic() + guides(fill = guide_legend(reverse=TRUE)) + 
  theme(axis.title.x = element_text(size=13),axis.title.y = element_text(size=13),legend.title.align=0.5, legend.title = element_text(size=13),axis.text = element_text(size=11),axis.text.x = element_text(vjust=1),legend.text = element_text(size=11)) +
...
hist(pw_comp$ITS1_Shannon_Diversity) # NOT normally distributed
hist(pw_comp$DustComplexity) # somewhat normally distributed
chisq.test(pw_comp$ITS1_Shannon_Diversity, pw_comp$DustComplexity)

its1.fit1<-lm(DustComplexity ~ ITS1_Shannon_Diversity, data=pw_comp) %>% 
...
#fig.its1.fit2<-ggplot(pw_comp, aes(x = ITS1_Shannon_Diversity, y = DustComplexity)) + 
...
hist(pw_comp$ITS1_Species_Richness)
its1.sr.fit1<-lm(DustComplexity ~ ITS1_Species_Richness, data=pw_comp) %>% 
  adjust_pvalue(method="bonferroni")
## ^ went with linear regression because Shannon diversity and dust complexity are continuous data, despite not being normally distributed
...
its1.sr.fit2<-lm(DustComplexity ~ ITS1_Species_Richness, data=pw_comp) %>% 
  adjust_pvalue(method="bonferroni")
summary(its1.sr.fit2)
coef(summary(its1.sr.fit2))
p.adjust(coef(summary(its1.sr.fit2))[,4], method="bonferroni") # pvalue 

its1.sr.fit2<-glm(DustComplexity ~ ITS1_Species_Richness, data=pw_comp, family=poisson)
its1.sr.fit3<-glm.nb(DustComplexity ~ ITS1_Species_Richness, data=pw_comp)
...
hist(pw_comp$Bac_Shannon_Diversity)

Bac.fit2<-glm(DustComplexity ~ Bac_Shannon_Diversity, data=pw_comp, family=poisson)

summary(Bac.fit2)
dispersiontest(Bac.fit2) 
# null hypothesis is that equidispersion exists; alternative hypothesis is overdispersion
# if overdispersion, use negative binomial not Poisson
## Poisson distribution implies that the mean and variance are equal --> little dispersion\
# negative binomial means # of observations is not fixed, whereas binomial means observations are a fixed #
...
#Coefficients:
#  Estimate Std. Error t value Pr(>|t|)    
#(Intercept)            0.6271566  0.0517419  12.121  7.1e-13 ***
#  Bac_Shannon_Diversity -0.0006131  0.0003710  -1.653    0.109
p.adjust(coef(summary(Bac.fit1))[8], method="bonferroni") # pvalue 
bac.r.adj<-summary(Bac.fit1)$adj.r.squared
summary(aov(Bac.fit1))

plot(DustComplexity ~ Bac_Shannon_Diversity, data=pw_comp)
abline(Bac.fit1)

fig.Bac.fit1<-ggplot(pw_comp, aes(x = Bac_Shannon_Diversity, y = DustComplexity)) + 
  geom_point(aes(color=Elevation), size=3) + theme_classic() + saturation(scale_colour_gradientn(colours=fair_cols,limits=c(400,2700),breaks = c(500,1250,2000,2600),labels=c("400","1100","2000","2700")), 0.9) +
  ...
  theme(axis.title.x = element_text(size=13),axis.title.y = element_text(size=13),legend.title.align=0.5, legend.title = element_text(size=13),axis.text = element_text(size=11),axis.text.x = element_text(vjust=1),legend.text = element_text(size=11)) +
  stat_cor(label.y = 1, label.x=280) +
  stat_regline_equation(aes(label=paste(..eq.label.., ..adj.rr.label.., sep = "~~~~")),label.y = 1.05,label.x=280)
## use summary(Bac.fit1) to double check that stat_cor gives same p value as linear regression!

ggsave(fig.Bac.fit1,filename = "figures/DustComp_by_16S_Shan_Div_ALL_1.4.22.pdf", width=10, height=8, dpi=600)

### wanted to see what regression line looked at without outliers
mean(pw_comp[-7,]$Bac_Shannon_Diversity)
pw_comp.bacclean<-pw_comp[(pw_comp$Bac_Shannon_Diversity) < 300,]
max(pw_comp.bacclean$Bac_Shannon_Diversity)

Bac.fit.nooutlier<-lm(DustComplexity ~ Bac_Shannon_Diversity, data=pw_comp.bacclean) %>% 
  adjust_pvalue(method="bonferroni")
summary(Bac.fit.nooutlier)

#Coefficients:
#  Estimate Std. Error t value Pr(>|t|)    
#(Intercept)            0.6095499  0.0587964  10.367 4.32e-11 ***
#  Bac_Shannon_Diversity -0.0003894  0.0005075  -0.767    0.449

fig.Bac.fit1a<-ggplot(pw_comp.bacclean, aes(x = Bac_Shannon_Diversity, y = DustComplexity)) + 
  geom_point(aes(color=Elevation), size=3) + theme_classic() + saturation(scale_colour_gradientn(colours=fair_cols,limits=c(400,2700),breaks = c(500,1250,2000,2600),labels=c("400","1100","2000","2700")), 0.9) +
  stat_smooth(method = "lm", col = "black", se=FALSE, size=1)+ labs(title="Dust Complexity x 16S Shannon Diversity", color="Elevation (ft)")+ylab("Dust Complexity")+xlab("16S Shannon Diversity")+
  theme(axis.title.x = element_text(size=13),axis.title.y = element_text(size=13),legend.title.align=0.5, legend.title = element_text(size=13),axis.text = element_text(size=11),axis.text.x = element_text(vjust=1),legend.text = element_text(size=11)) +
  stat_cor(label.y = 1, label.x=200) +
  stat_regline_equation(aes(label=paste(..eq.label.., ..adj.rr.label.., sep = "~~~~")),label.y = 1.05,label.x=200)

ggsave(fig.Bac.fit1a,filename = "figures/DustComp_by_16S_Shan_Div_NoOutliers_1.4.22.pdf", width=10, height=8, dpi=600)

# DustComp x Bac Species Richness
hist(pw_comp$Bac_Species_Richness)

Bac.sr.fit2<-glm(DustComplexity ~ Bac_Species_Richness, data=pw_comp, family=poisson)

summary(Bac.sr.fit2)
dispersiontest(Bac.sr.fit2) 
# null hypothesis is that equidispersion exists; alternative hypothesis is overdispersion
# if overdispersion, use negative binomial not Poisson
## Poisson distribution implies that the mean and variance are equal --> little dispersion
# negative binomial means # of observations is not fixed, whereas binomial means observations are a fixed #

Bac.sr.fit1<-lm(DustComplexity ~ Bac_Species_Richness, data=pw_comp) %>% 
  adjust_pvalue(method="bonferroni")
summary(Bac.sr.fit1)
p.adjust(coef(summary(Bac.sr.fit1))[8], method="bonferroni") # pvalue 
bac.sr.r.adj<-summary(Bac.sr.fit1)$adj.r.squared
summary(aov(Bac.sr.fit1))

write.csv(Tuk1$`as.factor(Elevation)`,"data/Sierra_DustComplexity_Elevation_TukeyHSD_ANOVA_3.1.22.csv",row.names=TRUE)

#Coefficients:
#  Estimate Std. Error t value Pr(>|t|)    
#(Intercept)           7.712e-01  8.273e-02   9.323 3.16e-10 ***
# Bac_Species_Richness -5.119e-05  1.877e-05  -2.727   0.0107 *
  
plot(DustComplexity ~ Bac_Species_Richness, data=pw_comp)
abline(Bac.sr.fit1)

fig.Bac.sr.fit1<-ggplot(pw_comp, aes(x = Bac_Species_Richness, y = DustComplexity)) + 
  geom_point(aes(color=Elevation), size=3) + theme_classic() + saturation(scale_colour_gradientn(colours=fair_cols,limits=c(400,2700),breaks = c(500,1250,2000,2600),labels=c("400","1100","2000","2700")), 0.9) +
  ...
  theme(axis.title.x = element_text(size=13),axis.title.y = element_text(size=13),legend.title.align=0.5, legend.title = element_text(size=13),axis.text = element_text(size=11),axis.text.x = element_text(vjust=1),legend.text = element_text(size=11)) +
  stat_cor(label.y = 1, label.x=6000) +
  stat_regline_equation(aes(label=paste(..eq.label.., ..adj.rr.label.., sep = "~~~~")),label.y = 1.05,label.x=6000)
## use summary(Bac.sr.fit1) to double check that stat_cor gives same p value as linear regression!

ggsave(fig.Bac.sr.fit1,filename = "figures/DustComp_by_16S_Species_Rich_ALL_1.4.22.pdf", width=10, height=8, dpi=600)
...
## fit1 comes from previous section: fit1<-aov(DustComplexity ~ Elevation2, data=pw_comp)
Fitted <- fitted(fit1)
...
fit.14<-ggplot(pw_2014, aes(x = Elevation, y = DustComplexity, fill=Elevation)) + 
  geom_boxplot() + theme_classic() + guides(fill = guide_legend(reverse=TRUE)) + 
  theme(axis.title.x = element_text(size=13),axis.title.y = element_text(size=13),legend.title.align=0.5, legend.title = element_text(size=13),axis.text = element_text(size=11),axis.text.x = element_text(vjust=1),legend.text = element_text(size=11)) +
...
fit.14.0<-ggplot(pw_2014, aes(x = Elevation, y = DustComplexity, fill=Elevation)) + 
  geom_boxplot() + theme_classic() + guides(fill = guide_legend(reverse=TRUE)) + 
  theme(axis.title.x = element_text(size=13),axis.title.y = element_text(size=13),legend.title.align=0.5, legend.title = element_text(size=13),axis.text = element_text(size=11),axis.text.x = element_text(vjust=1),legend.text = element_text(size=11)) +
  labs(title="Dust Complexity x Elevation (2014)",fill="Elevation (ft)")+ylab("Dust Complexity")+xlab("Elevation (ft)")+scale_fill_manual(values=saturation(fair_cols, 0.9))+stat_compare_means(method = "anova",label.y=1.5) +stat_compare_means(comparisons = list(c(1,2), c(2,3),  c(1,3),  c(3,4),  c(2,4),  c(1,4)), method="t.test", hide.ns = TRUE,label = "p.signif")
...
fit.14b.0<-ggplot(pw_2014, aes(x = Elevation, y = DustComplexity, fill=Elevation)) + 
  geom_boxplot() + theme_classic() + guides(fill = guide_legend(reverse=TRUE)) + 
  theme(axis.title.x = element_text(size=13),axis.title.y = element_text(size=13),legend.title.align=0.5, legend.title = element_text(size=13),axis.text = element_text(size=11),axis.text.x = element_text(vjust=1),legend.text = element_text(size=11)) +
...
fig.its1.fit1.14<-ggplot(pw_2014, aes(x = ITS1_Shannon_Diversity, y = DustComplexity, color=Elevation)) + 
  geom_point(size=3) + theme_classic() + saturation(scale_colour_gradientn(colours=fair_cols), 0.9) +
  stat_smooth(method = "lm", col = "black", se=FALSE, size=1)+ labs(color="Elevation (ft)")+ylab("Dust Complexity")+xlab("ITS1 Shannon Diversity")+
  theme(axis.title.x = element_text(size=13),axis.title.y = element_text(size=13),legend.title.align=0.5, legend.title = element_text(size=13),axis.text = element_text(size=11),axis.text.x = element_text(vjust=1),legend.text = element_text(size=11))+
  stat_cor(label.y = 0.8,label.x=63) +
  stat_regline_equation(aes(label=paste(..eq.label.., ..adj.rr.label.., sep = "~~~~")),label.y = 0.85,label.x=63)

ggsave(fig.its1.fit1.14,filename = "figures/DustComp_by_ITS1_Shan_Div_2014_12.16.21.pdf", width=10, height=8, dpi=600)


# DustComp x ITS1 Species Richness
hist(pw_2014$ITS1_Species_Richness) #somewhat normally distributed! but still
its1.fit2.14<-lm(DustComplexity ~ ITS1_Species_Richness, data=pw_2014)
summary(its1.fit2.14)

#dispersiontest(its1.fit2.14) - only for GLM (Poisson)
# null hypothesis is that equidispersion exists; alternative hypothesis is overdispersion
# if overdispersion, use negative binomial not Poisson
## Poisson distribution implies that the mean and variance are equal --> little dispersion
# negative binomial means # of observations is not fixed, whereas binomial means observations are a fixed #
...
fig.its1.fit2.14<-ggplot(pw_2014, aes(x = ITS1_Species_Richness, y = DustComplexity, color=Elevation)) + 
  geom_point(size=3) + theme_classic() + saturation(scale_colour_gradientn(colours=fair_cols), 0.9) +
  stat_smooth(method = "lm", col = "black", se=FALSE, size=1)+ labs(color="Elevation (ft)")+ylab("Dust Complexity")+xlab("ITS1 Species Richness")+
  theme(axis.title.x = element_text(size=13),axis.title.y = element_text(size=13),legend.title.align=0.5, legend.title = element_text(size=13),axis.text = element_text(size=11),axis.text.x = element_text(vjust=1),legend.text = element_text(size=11))+
  stat_cor(label.y = 0.8,label.x=650) +
  stat_regline_equation(label.y = 0.85,label.x=650)
...
# Dust Comp x Bac Shannon diversity
hist(pw_2014$Bac_Shannon_Diversity) # not normally distributed, but continuous data...
...
fig.Bac.fit1.14<-ggplot(pw_2014, aes(x = Bac_Shannon_Diversity, y = DustComplexity, color=Elevation)) + 
  ...
  theme(axis.title.x = element_text(size=13),axis.title.y = element_text(size=13),legend.title.align=0.5, legend.title = element_text(size=13),axis.text = element_text(size=11),axis.text.x = element_text(vjust=1),legend.text = element_text(size=11)) +
  stat_cor(label.y = 0.8,label.x=325) +
  stat_regline_equation(label.y = 0.85,label.x=325)

ggsave(fig.Bac.fit1.14,filename = "figures/DustComp_by_16S_Shan_Div_2014_12.16.21.pdf", width=10, height=8, dpi=600)


# DustComp x Bac Species Richness
hist(pw_2014$Bac_Species_Richness) # normally distributed
...
fig.Bac.fit2.14<-ggplot(pw_2014, aes(x = Bac_Species_Richness, y = DustComplexity, color=Elevation)) + 
  ...
  theme(axis.title.x = element_text(size=13),axis.title.y = element_text(size=13),legend.title.align=0.5, legend.title = element_text(size=13),axis.text = element_text(size=11),axis.text.x = element_text(vjust=1),legend.text = element_text(size=11)) +
  stat_cor(label.y = 0.8,label.x=6700) +
  stat_regline_equation(label.y = 0.85,label.x=6700)

ggsave(fig.Bac.fit2.14,filename = "figures/DustComp_by_16S_Spec_Richness_2014_12.16.21.pdf", width=10, height=8, dpi=600)
...
fit.15<-ggplot(pw_2015, aes(x = Elevation, y = DustComplexity, fill=Elevation)) + 
  geom_boxplot() + theme_classic() + guides(fill = guide_legend(reverse=TRUE)) + 
  theme(axis.title.x = element_text(size=13),axis.title.y = element_text(size=13),legend.title.align=0.5, legend.title = element_text(size=13),axis.text = element_text(size=11),axis.text.x = element_text(vjust=1),legend.text = element_text(size=11)) +
...
fit.15.0<-ggplot(pw_2015, aes(x = Elevation, y = DustComplexity, fill=Elevation)) + 
  geom_boxplot() + theme_classic() + guides(fill = guide_legend(reverse=TRUE)) + 
  theme(axis.title.x = element_text(size=13),axis.title.y = element_text(size=13),legend.title.align=0.5, legend.title = element_text(size=13),axis.text = element_text(size=11),axis.text.x = element_text(vjust=1),legend.text = element_text(size=11)) +
  labs(title="Dust Complexity x Elevation (2015)",fill="Elevation (ft)")+ylab("Dust Complexity")+xlab("Elevation (ft)")+scale_fill_manual(values=saturation(fair_cols, 0.9))+stat_compare_means(method = "anova",label.y=1.5) +stat_compare_means(comparisons = list(c(1,2), c(2,3),  c(1,3),  c(3,4),  c(2,4),  c(1,4)), method="t.test", hide.ns = TRUE,label = "p.signif")
...
fit.15b.0<-ggplot(pw_2015, aes(x = Elevation, y = DustComplexity, fill=Elevation)) + 
  geom_boxplot() + theme_classic() + guides(fill = guide_legend(reverse=TRUE)) + 
  theme(axis.title.x = element_text(size=13),axis.title.y = element_text(size=13),legend.title.align=0.5, legend.title = element_text(size=13),axis.text = element_text(size=11),axis.text.x = element_text(vjust=1),legend.text = element_text(size=11)) +
...
fig.its1.fit1.15<-ggplot(pw_2015, aes(x = ITS1_Shannon_Diversity, y = DustComplexity, color=Elevation)) + 
  ...
  stat_smooth(method = "lm", col = "black", se=FALSE, size=1)+ labs(color="Elevation (ft)")+ylab("Dust Complexity")+xlab("ITS1 Shannon Diversity")+
  theme(axis.title.x = element_text(size=13),axis.title.y = element_text(size=13),legend.title.align=0.5, legend.title = element_text(size=13),axis.text = element_text(size=11),axis.text.x = element_text(vjust=1),legend.text = element_text(size=11))+
  stat_cor(label.y = 0.8,label.x=77) +
  stat_regline_equation(label.y = 0.85,label.x=77)

ggsave(fig.its1.fit1.15,filename = "figures/DustComp_by_ITS1_Shan_Div_2015_12.16.21.pdf", width=10, height=8, dpi=600)


# DustComp x ITS1 Species Richness
hist(pw_2015$ITS1_Species_Richness) #somewhat normally distributed! but still
its1.fit2.15<-lm(DustComplexity ~ ITS1_Species_Richness, data=pw_2015)
summary(its1.fit2.15)

#dispersiontest(its1.fit2.15) - only for GLM (Poisson)
# null hypothesis is that equidispersion exists; alternative hypothesis is overdispersion
# if overdispersion, use negative binomial not Poisson
## Poisson distribution implies that the mean and variance are equal --> little dispersion
# negative binomial means # of observations is not fixed, whereas binomial means observations are a fixed #
...
# Dust Comp x Bac Shannon diversity
hist(pw_2015$Bac_Shannon_Diversity) # not normally distributed, but continuous data...
...
ggsave(fig.Bac.fit1.15,filename = "figures/DustComp_by_16S_Shan_Div_2015_12.16.21.pdf", width=10, height=8, dpi=600)


# DustComp x Bac Species Richness
hist(pw_2015$Bac_Species_Richness) # normally distributed
...
ggsave(fig.Bac.fit2.15,filename = "figures/DustComp_by_16S_Spec_Richness_2015_12.16.21.pdf", width=10, height=8, dpi=600)
